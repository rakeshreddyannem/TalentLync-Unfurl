...
export default function CandidateCard({ candidate, onToggleShortlist, onDelete }) {
  ...
  return (
    <div className="group relative glass-card p-5 rounded-2xl bg-white border border-slate-200 hover:border-[#368dff]/50 transition-all duration-300 hover:shadow-md flex flex-col justify-between">
      
      {/* Top Header: Platform Badge, Shortlist Heart & Delete */}
      <div className="flex items-center justify-between mb-4">
        <span className={`px-2.5 py-1 rounded-full text-xs font-bold border flex items-center space-x-1.5 ${platformMeta.badgeClass}`}>
          {platformMeta.icon}
        ...
        <div className="flex items-center space-x-2">
          {/* Experience Level */}
          <span className={`px-2 py-0.5 rounded-md text-[10px] font-bold border uppercase tracking-wider ${expBadgeColor}`}>
          ...
          <button
            type="button"
            onClick={() => onToggleShortlist(candidate._id)}
          ...
          <button
            ...
            className="p-1.5 rounded-lg bg-slate-100 border border-slate-200 text-slate-400 hover:text-[#db3662] transition-colors opacity-70 group-hover:opacity-100"
      ...
      <div className="flex items-start space-x-3.5 mb-4">
        <img
          ...
          alt={candidate.name}
          className="w-14 h-14 rounded-2xl object-cover border border-slate-200 bg-slate-100 shrink-0 shadow-2xs"
          onError={(e) => {
      ...
      {/* Relevance Score Bar */}
      <div className="my-3 p-2.5 rounded-xl bg-slate-50 border border-slate-200">
      ...
      {candidate.skillTags && candidate.skillTags.length > 0 && (
        <div className="flex flex-wrap gap-1.5 my-2">
          {candidate.skillTags.slice(0, 5).map((skill, idx) => (
            <span
              key={idx}
              className="px-2.5 py-0.5 text-[11px] font-semibold rounded-md bg-slate-100 text-slate-700 border border-slate-200"
            >
              {skill}
            </span>
          ))}
          {candidate.skillTags.length > 5 && (
            <span className="px-2 py-0.5 text-[10px] font-bold text-slate-500 rounded-md bg-slate-100 border border-slate-200">
              +{candidate.skillTags.length - 5}
            </span>
          )}
        </div>
      ...
      <div className="pt-3 mt-3 border-t border-slate-200 flex items-center justify-between text-xs text-slate-500">
        ...
        <a
          ...
          target="_blank"
          rel="noreferrer"
          className="text-[#368dff] hover:text-[#257ce6] font-bold flex items-center space-x-1 transition-colors"
        >
