...
export default function CandidateFilterBar({
  ...
  const handlePlatform = onPlatformChange || setSelectedPlatform || (() => {});
  const handleMinScore = onMinScoreChange || setMinScore || (() => {});
  const handleShortlist = onShortlistToggle || setShortlistOnly || (() => {});
  ...
  return (
    <div className="w-full glass-panel p-5 rounded-3xl border border-slate-200 space-y-4 my-6 bg-white shadow-sm">
      ...
      <div className="flex flex-col md:flex-row items-stretch md:items-center justify-between gap-3">
        ...
        <div className="relative flex-1">
          ...
          <input
            ...
            className="w-full pl-10 pr-4 py-2.5 bg-slate-50 border border-slate-200 rounded-2xl text-xs text-slate-900 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-[#368dff] focus:bg-white transition-all font-medium"
          />
        </div>
        ...
        <div className="flex items-center space-x-3 self-end md:self-auto">
          ...
          <button
            type="button"
            onClick={() => handleShortlist(!shortlistOnly)}
            className={`px-3.5 py-2 rounded-2xl text-xs font-extrabold border flex items-center space-x-1.5 transition-all cursor-pointer ${
              shortlistOnly
          ...
          <div className="flex items-center p-1 rounded-2xl bg-slate-100 border border-slate-200">
            <button
              ...
              className={`p-1.5 rounded-xl text-xs transition-colors cursor-pointer ${
                viewMode === 'grid' ? 'bg-[#368dff] text-white shadow-sm font-extrabold' : 'text-slate-500 hover:text-slate-800'
              }`}
              title="Grid View"
            >
            ...
            <button
              ...
              className={`p-1.5 rounded-xl text-xs transition-colors cursor-pointer ${
                viewMode === 'table' ? 'bg-[#368dff] text-white shadow-sm font-extrabold' : 'text-slate-500 hover:text-slate-800'
              }`}
              title="Table View"
            >
          ...
          <button
            ...
            className="px-4 py-2.5 bg-[#00a962] hover:bg-[#008e52] text-white text-xs font-extrabold rounded-2xl shadow-md shadow-[#00a962]/20 flex items-center space-x-1.5 transition-all cursor-pointer"
          >
      ...
      <div className="flex flex-col lg:flex-row items-start lg:items-center justify-between gap-4 pt-3 border-t border-slate-200">
          ...
          {platforms.map((p) => {
            ...
            return (
              <button
                ...
                className={`px-3 py-1.5 rounded-xl text-xs font-extrabold border flex items-center space-x-1.5 transition-all cursor-pointer ${
                  active
        ...
        {/* Min Score Slider */}
        <div className="flex items-center space-x-3 w-full lg:w-auto bg-slate-50 px-4 py-2 rounded-2xl border border-slate-200">
